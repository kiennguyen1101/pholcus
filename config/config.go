...
const (
	...
	NAME      string = "Pholcus Ghost Spider Data Collection"        // 软件名
...
func init() {
	...
	cache.Task = &cache.AppConf{
		...
		Master:         setting.String("run::master"),                         // Server(主节点)地址，不含端口
		ThreadNum:      setting.DefaultInt("run::thread", thread),             // 全局最大并发量
		Pausetime:      setting.DefaultInt64("run::pause", pause),             // Pause duration/ms(随机: Pausetime/2 ~ Pausetime*2)
